...
export default function App() {
  const BACKEND_URL = 'https://chikitsalya-backend.onrender.com'; // Adjust to your Render backend
  ...
  // 1. Initialize Location & Initial Fetch
  useEffect(() => {
    (async () => {
  ...
  const handleQuery = async () => {
    ...
    try {
      const res = await fetch(`${BACKEND_URL}/predict`, {
        ...
        body: JSON.stringify({ query: query, lang: 'en' }),
      });
      const data = await res.json();
      setAnswer(data.reply || 'No response from backend');
    } catch (err) {
      setAnswer('Error connecting to backend AI service.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchHospitals = async (lat, lon) => {
    try {
      const res = await fetch(`${BACKEND_URL}/api/hospitals?lat=${lat}&lon=${lon}`);
      const data = await res.json();
      setHospitals(data);
          ...
          {activeTab === 'assistant' ? (
            <ScrollView contentContainerStyle={styles.content}>
              <View style={styles.inputBox}>
                ...
                <TouchableOpacity style={styles.predictBtn} onPress={handleQuery} disabled={loading}>
                  {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnTextMain}>Get Guidance 🧠</Text>}
                </TouchableOpacity>
              </View>
            ...
            <View style={{ flex: 1 }}>
              ...
              <View style={{ height: 60 }}>
                ...
                >
                  {['All', 'Multispeciality', 'Dentistry', 'Surgeon', 'Clinic'].map(cat => (
                    <TouchableOpacity 
                      key={cat} 
              ...
              <FlatList
                ...
                ListEmptyComponent={() => (
                  <View style={styles.emptyBox}>
                    <Text style={styles.emptyText}>No facilities found for this selection. 📡</Text>
                  </View>
                )}
              />
            </View>
...
const styles = StyleSheet.create({
  ...
  activeTab: { backgroundColor: '#0ea5e9 shadowColor: #0ea5e9', shadowOpacity: 0.3, shadowRadius: 5 },
